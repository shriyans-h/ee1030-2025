...
import os
import sys
dot = ctypes.CDLL('./dot.so')
dot.dot.argtypes = [
     ctypes.POINTER(ctypes.c_double),
     ctypes.POINTER(ctypes.c_double),
     ctypes.c_int
]
dot.dot.restype = ctypes.c_double

d1 = np.array([7, -5, 1], dtype=np.float64)
d2 = np.array([1, 2, 3], dtype=np.float64)
m = len(d1)

d = dot.dot(
    d1.ctypes.data_as(ctypes.POINTER(ctypes.c_double)),
    d2.ctypes.data_as(ctypes.POINTER(ctypes.c_double)),
    m
)
