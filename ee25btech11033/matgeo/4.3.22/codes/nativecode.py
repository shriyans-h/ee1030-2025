...
# Coordinates updated from the provided images
A = np.array([1, -5])
B = np.array([-4, 5])
P_dividing = np.array([-3/2, 0]) # This is -1.5
...
# We can take the first component of the ratio vector since they should be equal
ratio_val = ratio[0]
print(f'Point {tuple(P_dividing)} divides the line AB in the ratio: {ratio_val}:1')
...
point_labels = ['A (1,-5)', 'B (-4,5)', 'P (-1.5,0)']
...
plt.xlabel('$x$')
plt.ylabel('$y$')
# Update the title with the new point and calculated ratio
plt.legend(loc='best')
plt.grid(True)
plt.axis('equal')
