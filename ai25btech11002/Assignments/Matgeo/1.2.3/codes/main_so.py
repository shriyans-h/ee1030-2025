...
C = np.array([1,-6,-7],dtype=np.float64)
R = np.array([0,0,0],dtype=np.float64)
# Load shared library
lib = ctypes.CDLL("./cfunc.so")

# Set argtypes and restype
lib.add_three_arrays.argtypes = [
    np.ctypeslib.ndpointer(dtype=np.float64, ndim=1, shape=(3,), flags="C_CONTIGUOUS"),
    np.ctypeslib.ndpointer(dtype=np.float64, ndim=1, shape=(3,), flags="C_CONTIGUOUS"),
    np.ctypeslib.ndpointer(dtype=np.float64, ndim=1, shape=(3,), flags="C_CONTIGUOUS"),
    np.ctypeslib.ndpointer(dtype=np.float64, ndim=1, shape=(3,), flags="C_CONTIGUOUS"),
]
lib.add_three_arrays.restype = None

# Call the C function
lib.add_three_arrays(A, B, C, R)
